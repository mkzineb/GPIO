...
PortSpecHeader : PORT LocatedID LPAREN PortPart RPAREN  
    { Sem.add_spec (fst $2) (Sem.make_port $2 (fst $4) (snd $4) []); $2 }
;
...
|	IF Expr THEN Sequence ELSIF Expr THEN Sequence OptionalElse ENDIF
		{ Irg.IF_STAT ($2, $4, Irg.IF_STAT ($6, $8, $9)) }
...
|	IF Expr THEN Expr ELSIF Expr ENDIF
		{ handle_expr (fun _ -> Sem.make_if_expr $2 $4 $6) }
|	IF error
