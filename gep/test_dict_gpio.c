...
// READ - WRITE
    ...
    csim_word_t  gpio_read_LCKR(csim_inst_t *inst, int indX) {
        gpio_inst_t * gpio_inst = ( gpio_inst_t *)inst;
        return  gpio_inst->LCKR;
    }

  
csim_reg_t  gpio_registers[3] = {
        {"MODER", 0x1, 32, 1, 1, 0, CSIM_INT, NULL, NULL,  gpio_read_MODER,  gpio_write_MODER, NULL, NULL},
        {"IDR", 0x1, 32, 1, 1, 0, CSIM_INT, NULL, NULL,  gpio_read_IDR,  gpio_write_IDR, NULL, NULL},
        {"BSRR", 0x1, 32, 1, 1, 0, CSIM_INT, NULL, NULL,  gpio_read_BSRR,  gpio_write_BSRR, NULL, NULL},
        {"ODR", 0x1, 32, 1, 1, 0, CSIM_INT, NULL, NULL,  gpio_read_ODR,  gpio_write_ODR, NULL, NULL},
        {"LCKR", 0x1, 32, 1, 1, 0, CSIM_INT, NULL, NULL,  gpio_read_LCKR,  gpio_write_LCKR, NULL, NULL},
...
void  gpio_construct(csim_inst_t *inst) {
    gpio_inst_t * gpio_inst = ( gpio_inst_t *)inst;
     ...
     gpio_inst->gpio_port = inst->ports;
}
...
int main() {
    ...
    for (int i=0 ; i<NB_PINS ; i++){
            update_pins_global(gpio_instance->gpio_port, CSIM_ELECTRIC, pins_value, i);
