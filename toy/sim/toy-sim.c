 ...
 */
void copy_options_to_toy_env(toy_env_t *env, init_options *opt)
{
	env->argc = opt->argc;
...
FILE* load_profiling_file(char* profiling_file_name, int inst_stat[])
{
    ...
    if(profile_id == NULL)
    {
        ...
        for(i = 0; i<TOY_INSTRUCTIONS_NB; i++)
...
void write_profiling_file(FILE* profile_id, int inst_stat[])
{
	...
	}tmp, entries[TOY_INSTRUCTIONS_NB];
	...
	for(i = 0; i<TOY_INSTRUCTIONS_NB; i++)
	{
		for(j = i+1; j<TOY_INSTRUCTIONS_NB; j++)
		{
    ...
    for(i = 0; i<TOY_INSTRUCTIONS_NB; i++)
    {
        fprintf(profile_id, "%s %d %d\n",
        	toy_get_string_ident(entries[i].id),	// instruction name
...
int main(int argc, char **argv) {
    toy_state_t *state = 0;
    toy_platform_t *platform = 0;
    toy_loader_t *loader = 0;
    toy_address_t addr_start = 0;
    toy_address_t addr_exit = 0;
	char *c_ptr = 0;
    char *profiling_file_name;
    char *profiling_file_path = NULL;
    FILE *profile_id=0;
    int inst_stat[TOY_INSTRUCTIONS_NB];
	...
	/* open the exec file */
    loader = toy_loader_open(argv[prog_index]);
	if(loader == NULL) {
	...
	if(!is_start_given)
        addr_start = toy_loader_start(loader);
	if(verbose)
		printf("START=%08x\n", addr_start);
	...
	if (!is_exit_given) {
		...
		/* search symbol _exit */
        for(sym_exit = 0; sym_exit < toy_loader_count_syms(loader); sym_exit++) {
            toy_loader_sym_t data;
            toy_loader_sym(loader, sym_exit, &data);
    ...
    copy_options_to_toy_env(toy_get_sys_env(platform), &options);

	/* load the image in the platform */
    if (toy_load_platform(platform, argv[prog_index]) == -1) {
    ...
    if(profile)
    {
        ...
        if( profiling_file_path != NULL){
			profiling_file_name = (char*)alloca(sizeof(char)* (strlen(profiling_file_path) + strlen(TOY_PROC_NAME) + 10));
			...
			strcat(profiling_file_name, TOY_PROC_NAME);
		}else{
			profiling_file_name = (char*)alloca(sizeof(char)* strlen(TOY_PROC_NAME) + 10);
			strcpy(profiling_file_name, TOY_PROC_NAME);
		}
...
#	endif
    ...
    if(!verbose && !profile && !valid)
    {
			...
			if(fast_sim)
				inst_cnt += toy_run_and_count_inst(sim);
			else
			{
				while(addr_exit != state->TOY_PC_NAME
				&& !toy_is_sim_ended(sim))
				{
					toy_step(sim);
					inst_cnt++;
				}
	...
	else
	{
        ...
        if(valid) {
			...
			toy_output_header_valid(vout);
		}

		/* perform the simulation */
        while(!toy_is_sim_ended(sim)) {
            inst = toy_next_inst(sim);
            if( profile )
                inst_stat[inst->ident]++;
            if(verbose) {
                toy_disasm(buffer, inst);
                fprintf(stderr, "%08x: %s\n", toy_next_addr(sim),  buffer);
            }
            if(valid)
				toy_output_state_valid(state, vout);
            toy_free_inst(inst);
            toy_step(sim);
            if(dump) {
				toy_dump_state(state, stderr);
				fputc('\n', stderr);
			}
			inst_cnt++;
		}
