...
//#define TOY_MEM_SPY		// define it to get memory action spy feature
...
/**
 * @typedef toy_memory_t
 * This type is used to represent a memory space.
 * @ingroup memory
 */
typedef struct toy_memory_t
{
...
#	endif
#ifdef TOY_MEM_SPY
	toy_mem_spy_t spy_fun;	/** spy function */
 ...
 */
static void toy_mem_default_spy(toy_memory_t *mem, toy_address_t addr, toy_size_t size, toy_access_t access, void *data) {
}
#endif
...
toy_memory_t* toy_mem_new(void) {
	...
	/* initialize spy */
#	ifdef TOY_MEM_SPY
		mem->spy_fun = toy_mem_default_spy;
		mem->spy_data = 0;
#	endif

	return (toy_memory_t*) mem;
}
 ...
 */
static page_entry_t* mem_get_page(memory_64_t* mem, toy_address_t addr)
{
    uint32_t      hash1;
    page_entry_t* tmp;
    page_entry_t* entry;
...
toy_memory_t* toy_mem_copy(toy_memory_t* memory)
{
    ...
    memory_64_t* target = (memory_64_t*)toy_mem_new();
 ...
 * @ingroup memory
 */
void toy_mem_write(toy_memory_t* memory, toy_address_t address, void* buffer, size_t size)
{
    assert(size > 0);
	int i;
...
#	ifdef TOY_MEM_SPY
    	mem->spy_fun(mem, address, size, toy_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void toy_mem_read(toy_memory_t *memory, toy_address_t address, void *buffer, size_t size)//////////////////////////////
{
    assert(size > 0);
	int i;
    uint32_t      offset = FMOD(address, MEM_PAGE_SIZE);
...
#	ifdef TOY_MEM_SPY
    	mem->spy_fun(mem, address, size, toy_access_read, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
uint8_t toy_mem_read8(toy_memory_t *mem, toy_address_t address)
{
    page_entry_t* pte    = mem_get_page(mem, address);
    toy_address_t offset = FMOD(address, MEM_PAGE_SIZE);
    uint8_t r;
...
#	ifdef TOY_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(r), toy_access_read, mem->spy_data);
#	endif
    return r;
}
 ...
 * @ingroup memory
 */
uint16_t toy_mem_read16(toy_memory_t *mem, toy_address_t address) {
...
#   endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        ...
        toy_mem_read(mem, address, val.bytes, 2);
        r = val.half;
    }
 ...
 * @ingroup memory
 */
uint32_t toy_mem_read32(toy_memory_t *mem, toy_address_t address)
{
...
#   endif
    ...
    else
    {
		...
		toy_mem_read(mem, address, val.bytes, 4);
        r = val.word;
    }
 ...
 * @ingroup memory
 */
uint64_t toy_mem_read64(toy_memory_t *mem, toy_address_t address) {
...
#   if HOST_ENDIANNESS != TARGET_ENDIANNESS
    ...
    else
    {
        ...
        toy_mem_read(mem, address, val.bytes, 8);
        r = val.dword;
    }
 ...
 * @ingroup memory
 */
float toy_mem_readf(toy_memory_t *mem, toy_address_t address)
{
	union {
		uint32_t i;
		float f;
	} val;
    val.i = toy_mem_read32(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
double toy_mem_readd(toy_memory_t *mem, toy_address_t address)
{
	union {
		uint64_t i;
		double f;
	} val;
    val.i = toy_mem_read64(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
long double toy_mem_readld(toy_memory_t *memory, toy_address_t address)
{
 ...
 * @ingroup memory
 */
void toy_mem_write8(toy_memory_t* mem, toy_address_t address, uint8_t val)
{
    toy_address_t offset;
    page_entry_t*   pte;
    pte    = mem_get_page(mem, address);
...
#	ifdef TOY_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(val), toy_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void toy_mem_write16(toy_memory_t* mem, toy_address_t address, uint16_t val)
{
    toy_address_t offset;
    uint16_t* q;
...
# endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        *q = val;
		...
		toy_mem_write(mem, address, p->bytes, 2);
	}
 ...
 * @ingroup memory
 */
void toy_mem_write32(toy_memory_t* mem, toy_address_t address, uint32_t val) {
	uint32_t* q;
    toy_address_t offset;
    page_entry_t*   pte;
...
#   endif
    ...
    if(!((offset & 0x3) | ((offset + 3) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        toy_mem_write(mem, address, p->bytes, 4);
	}
 ...
 * @ingroup memory
 */
void toy_mem_write64(toy_memory_t *mem, toy_address_t address, uint64_t val)
{
    toy_address_t offset;
    uint64_t* q;
...
#   endif
    ...
    if(!((offset & 0x7) | ((offset + 7) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        toy_mem_write(mem, address, p->bytes, 8);
	}
 ...
 * @ingroup memory
 */
void toy_mem_writef(toy_memory_t *memory, toy_address_t address, float val)
{
	union {
		uint32_t i;
		float f;
	...
	v.f = val;
    toy_mem_write32(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void toy_mem_writed(toy_memory_t *memory, toy_address_t address, double val)
{
	union {
		uint64_t i;
		double f;
	...
	v.f = val;
    toy_mem_write64(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void toy_mem_writeld(toy_memory_t *memory, toy_address_t address, long double val)
{
	assertp(0, "not implemented");
}
