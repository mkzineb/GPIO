...
#	endif
	...
	/* allocate handler */
	TRACE;
	loader = (toy_loader_t *)malloc(sizeof(toy_loader_t));
	if(loader == NULL) {
		errno = ENOMEM;
		close(elf);
		//!!DEBUG!!
...
{
	assert(loader);
	/*toy_loader_t *backup = malloc(sizeof(toy_loader_t));*/
...
void toy_loader_load(toy_loader_t *loader, toy_platform_t *pf) {
	...
	toy_memory_t *memory = toy_get_memory(pf, TOY_MAIN_MEMORY);
    ...
    while(ptr_tex != NULL)
    {
        TRACE;
        if(ptr_tex->size)
			toy_mem_write(memory, ptr_tex->address, ptr_tex->bytes, ptr_tex->size);
    ...
    while(ptr != NULL)
    {
		TRACE;
		if(ptr->size)
			toy_mem_write(memory, ptr->address, ptr->bytes, ptr->size);
		ptr = ptr->next;
	}

	/* initialize BSS part */
#	ifdef TOY_NOBITS_INIT
		for(i = 0; i < loader->Tables.sechdr_tbl_size; i++) {
			...
			if(sec->sh_type == SHT_NOBITS) {
				...
				for(j = 0; j < sec->sh_size; j += sizeof(uint32_t))
					toy_mem_write32(memory, sec->sh_addr + j, 0);
			}
		}
#	endif
}
	...
	case SHT_PROGBITS:
		if(s->sh_flags & SHF_ALLOC) {
			if(s->sh_flags & SHF_EXECINSTR)
				data->type = TOY_LOADER_SECT_TEXT;
			else
				data->type = TOY_LOADER_SECT_DATA;
		}
		else
			data->type = TOY_LOADER_SECT_BSS;
		break;
	default:
		data->type = TOY_LOADER_SECT_UNKNOWN;
		break;
	}
...
void toy_loader_sym(toy_loader_t *loader, int sym, toy_loader_sym_t *data) {
	assert(sym < toy_loader_count_syms(loader));
	Elf32_Sym *s;
	...
	case STB_LOCAL: data->bind = TOY_LOADER_LOCAL; break;
	case STB_GLOBAL: data->bind = TOY_LOADER_GLOBAL; break;
	case STB_WEAK: data->bind = TOY_LOADER_WEAK; break;
	default: data->bind = TOY_LOADER_NO_BINDING; break;
	}

	/* get the type */
	switch(ELF32_ST_TYPE(s->st_info)) {
	case STT_FUNC: data->type = TOY_LOADER_SYM_CODE; break;
	case STT_OBJECT: data->type = TOY_LOADER_SYM_DATA; break;
	default: data->type = TOY_LOADER_SYM_NO_TYPE; break;
	}
}
...
toy_address_t toy_brk_init(toy_loader_t *loader)
{
	...
	among all loadable segments */
     ...
     for (i = 0; i < n; i++)
     {
	     ...
	     {
		     toy_address_t new_brk = seg_tbl[i].p_vaddr + seg_tbl[i].p_memsz;
		     if (new_brk > brk_init)
			     brk_init = new_brk;
	     }
     }
